document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupTrendingNav() {
    ...
    // Check if Trending link already exists
    const existingTrending = navMenu.querySelector('a[href*="/trending"], a[href*="trending/"]');
    if (existingTrending) {
      if (currentPath.startsWith('/trending')) {
        existingTrending.classList.add('active');
      }
      return;
    }

    // Find the Contact Us link - it might be the last .nav-link
    let contactLink = navMenu.querySelector('.nav-link[href*="contact"]');
    if (!contactLink) {
      // Fallback: find all nav-links and use the last simple link (not a dropdown)
    ...
    trendingLink.className = 'nav-link';
    trendingLink.href = '/trending/';
    trendingLink.textContent = 'Trending';

    contactLink.insertAdjacentElement('afterend', trendingLink);
  ...
  async function setupTrendingPage() {
    ...
    function setPageMetadata(categoryKey) {
      ...
      const pageName = categoryKey === 'all' ? 'Trending' : `${label} Trending`;
    ...
    function renderErrorState() {
      ...
      const empty = document.createElement('div');
      empty.className = 'trending-empty-state';
      empty.textContent = 'Trending briefs are being refreshed. Please try again shortly.';
      listContainer.appendChild(empty);
    }
    ...
    try {
      ...
      if (updatedLabel && data.updatedDate) {
        updatedLabel.textContent = `Updated ${data.updatedDate}`;
      }
  ...
  const siteSearchPages = [
    ...
    { title: 'Trending Technology & AI Insights', category: 'Trending', url: 'trending/', terms: 'trending latest ai business technology news insights sme singapore asia automation productivity cybersecurity cloud google workspace microsoft 365 digital presence decision makers' },
    { title: 'Trending AI Insights', category: 'Trending', url: 'trending/ai/', terms: 'artificial intelligence ai assistants agents chatgpt business ai adoption automation summaries' },
    { title: 'Trending Business Technology', category: 'Trending', url: 'trending/business/', terms: 'business technology sme trends singapore asia digital adoption operations productivity ai governance' },
    { title: 'Trending Automation Insights', category: 'Trending', url: 'trending/automation/', terms: 'automation workflow crm leads whatsapp invoices quotations approvals business process automation' },
    { title: 'Trending Productivity Tools', category: 'Trending', url: 'trending/productivity/', terms: 'productivity tools google workspace microsoft 365 documents tasks collaboration meetings' },
    { title: 'Trending Cybersecurity Briefs', category: 'Trending', url: 'trending/security/', terms: 'cybersecurity security ransomware advisories patching backups mfa website security cloud security' },
    { title: 'Trending Websites & Digital Presence', category: 'Trending', url: 'trending/websites/', terms: 'websites digital presence seo analytics search console ga4 local visibility website optimization' },
  ...
  function setupSiteSearch() {
    if (!navActions || document.getElementById('siteSearchPanel')) return;
    ...
    if (menuButton && navActions.parentNode) {
  ...
  function getAxonAgentFallbackReply(question) {
    ...
    if (text.includes('google') || text.includes('seo') || text.includes('found')) {
      return 'Start with indexing basics: page titles, sitemap, robots.txt, Search Console, analytics and whether Google can crawl the page. Send the website URL in the form and Axon can check it.';
    }
    if (text.includes('shopify') || text.includes('checkout') || text.includes('product')) {
      return 'For Shopify, send the store URL, the affected product or checkout step, and a screenshot if available. Axon can check theme, domain, checkout, analytics and app issues.';
    }
    if (text.includes('wix') || text.includes('form') || text.includes('email')) {
      return 'For form or email issues, include the website URL, where the form should send, and whether email uses Google Workspace, Microsoft 365 or hosting mail. Axon can check routing and SMTP.';
    }
    if (text.includes('ai') || text.includes('publish') || text.includes('app')) {
      return 'For AI-built sites or apps, send the project link, code/export if available, and what is blocking launch. Axon can review hosting, domain, backend, database and API setup.';
    }
    if (text.includes('hosting') || text.includes('domain') || text.includes('ssl')) {
      return 'For hosting, domain or SSL help, send the domain name, hosting provider and current error message. Axon can check DNS, SSL, redirects and server setup.';
    }
    if (text.includes('hacked') || text.includes('malware') || text.includes('security')) {
      return 'For security issues, avoid editing the site until backups and access are checked. Send the website URL, hosting login path and any warning message so Axon can advise next steps.';
    }

    return 'Please share the website or platform, what you expected to happen, and what is happening now. The fastest path is to send those details using the form or WhatsApp.';
  }
  ...
  async function askAxonAgent(question) {
    ...
    const reply = kimReply || getAxonAgentFallbackReply(cleanedQuestion);
